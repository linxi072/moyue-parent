#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# 墨阅小说网 · 全链路冒烟测试（单体直连 moyue-app :8080）
# 覆盖：登录取 token -> IM 会话/消息 -> 积分商城（含 2 个失败场景）-> 博客（含点赞幂等）
# 前置：
#   1) 原生启动 MySQL / Redis（项目硬性约束：禁用 Docker，见 docs/不可忽视条件.md；ES 仅检索域可选）
#   2) ./scripts/build.sh
#   3) ./scripts/start-all.sh，确认单体 moyue-app 已监听 :8080
