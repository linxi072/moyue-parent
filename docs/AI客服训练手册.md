# AI 客服训练手册（墨阅小说网 · moyue-ai）

> 适用版本：P1-1「AI 客服真实化」落地后（提交 `f1ba0d6`，2026-09-16）
> 面向对象：运营训练师 / 客服负责人 / 接入大模型的运维
> 配套代码：`moyue-modules/moyue-ai`、`moyue-modules/moyue-search`（RAG）、`moyue-api/moyue-api-search`（Feign 契约）
...
- 大模型适配层（LLM）：配置驱动，接入 OpenAI 兼容网关后真实生成。
...
- 大模型接入是**配置驱动**：运维在 yml / Nacos 配 `moyue.ai.llm.enabled=true` + `apiKey` + `endpoint` 即生效，业务代码零改动；缺密钥或未启用时**自动降级**到关键词规则，不报错。
- 转人工是**会话内提示**（"请回复「人工」"），**不是跨服务工单派单**；人工坐席接入不在本模块范围内。
- 未做敏感词前置机审；RAG 召回内容来自内部问答库，经 Feign fallback 安全降级。
...
组装 ReplyContext { content, history(最近10轮), ragContext(RAG召回) }
  │
  ▼
主引擎 LlmReplyEngine.reply()  ── enabled 且 apiKey 非空 ──► 调 OpenAI 兼容网关
  │  返回 null（未启用/无密钥/网关异常/空响应）
  ▼
...
落「助手消息」(role=2)
  │
  ▼
异步 indexQaAsync()  ► 推 moyue-search 建 ES 索引 moyue-qa（失败仅 warn，不影响对话）
```
...
| `LlmGatewayClient` | OpenAI 兼容 `/v1/chat/completions` 真实外呼 |
| `LlmProperties` | `moyue.ai.llm.*` 配置 |
| `QaSearchClient`（Feign） | RAG 召回 → `moyue-search` `/api/v1/internal/search/qa/retrieve` |
...
来源：运维接入 LLM 网关后，由 `LlmReplyEngine` 实时生成。
...
降级条件：未启用 / `apiKey` 为空 / 网关异常 / 返回空 → 返回 `null content` → 级联关键词引擎。

### 3.3 RAG 知识库（ES 自动沉淀 + 全量重建）
来源：用户与客服的**真实对话**在每轮落库成功后异步写入 ES 索引 `moyue-qa`（IK 分词）。

- 召回：`AiService` 经 `QaSearchClient`（Feign，`moyue-search`）调 `/api/v1/internal/search/qa/retrieve?question=`，取 `ragTopK`(默认 3) 条 `"Q:…\nA:…"` 片段拼为 `ragContext` 注入大模型。
- 安全降级：`moyue-search` 未注册 / ES 不可用 / 召回异常 → `QaSearchClient` fallback 返回空 → `ragContext=null`，**对话照常进行，仅失去知识增强**。
- 全量重建：管理端可经 `AiService.pageQaForIndex(page,size)` 拉取全量问答对（role=1 提问与其后 role=2 回复配对），重建 `moyue-qa` 索引。删除会话时须同步调 `searchIndexClient.removeQaBySession(sessionId)` 清理索引（当前无业务入口，见代码 TODO）。
...
1. 编辑 `moyue-modules/moyue-ai/.../engine/KeywordRuleReplyEngine.java` 的 `RULES` 静态块；
2. 新增一条：`RULES.put(new String[]{"关键词1","关键词2"}, "标准话术");`（LinkedHashMap 顺序即优先级，高频规则靠前）；
3. 话术约束：简洁、友好、准确，仅谈平台业务；超出范围的话术引导转人工；
4. 改完跑 `moyue-ai` 模块测试（见运维手册 §5）确认无回归；
5. 编译部署后生效。
...
其它可调项：`model`（模型）、`timeoutSeconds`（网关超时）、`maxHistory`（历史轮次）、`temperature`（代码固定 0.3，如需调需改 `LlmGatewayClient`）。
...
- **知识库命中**：可经 `moyue-search` 的 `moyue-qa` 检索接口直接验证某问题能否召回相关片段。
- **回归基线**：`moyue-ai` 模块 10 例单测 + `moyue-search` 70 例（含 RAG 召回），全绿方可发布。
...
- [ ] ES：`moyue-search` 的 `moyue-qa` 索引可用（RAG 依赖）；不可用则自动降级。
- [ ] 关键词规则审阅：高频业务已覆盖；坏话术已修订。
- [ ] systemPrompt 人设核对。
- [ ] `moyue-ai` + `moyue-search` 测试全绿。
- [ ] 转人工提示文案与坐席在线时段一致。
...
A：可能 `apiKey` 为空 / `enabled=false`（直接降级），或网关超时/异常（每轮降级并 warn）。先查 `moyue.ai.llm.*` 配置与网关连通性。

**Q：RAG 为什么不生效？**
A：ES/`moyue-search` 未启动或 `moyue-qa` 索引为空；召回失败仅 warn 不阻断。先用检索接口验证 `moyue-qa` 是否有数据。

**Q：能否在后台直接加关键词规则？**
A：不能。规则是 Java 硬编码，需按 §4.1 改代码部署。长尾问题建议优先用 LLM+RAG。

**Q：转人工为什么只是提示，没有工单？**
A：当前转人工是会话内提示，跨服务工单派单不在本模块范围（见 §1 边界）。
