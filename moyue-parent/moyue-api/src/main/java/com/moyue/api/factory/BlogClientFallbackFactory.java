...
/**
 * BlogClient 降级工厂：目标服务 moyue-social 不可用时返回统一降级响应（R.fail 40002），
...
public class BlogClientFallbackFactory implements FallbackFactory<BlogClient> {
    ...
    private static final Logger log = LoggerFactory.getLogger(BlogClientFallbackFactory.class);

    @Override
    @SuppressWarnings("unchecked")
    public BlogClient create(Throwable cause) {
        log.error("Feign 降级：{} 调用失败 -> {}", "moyue-social", cause.getMessage());
        return (BlogClient) Proxy.newProxyInstance(
                BlogClient.class.getClassLoader(),
                new Class<?>[]{BlogClient.class},
                (proxy, method, args) -> {
