...
> P1-2 已于 2026-09-16 落地：短信/推送桩替换为「配置驱动的真实 HTTP 外呼」，`UserDTO`/`user` 表补 `email`/`device_token`，邮件默认按用户资料解析、短信/推送按 `device_token` 真实送达；缺密钥/未启用时各渠道优雅降级为 `pending`，不阻断主链路。提交 `e6b4b81`（13 文件 +350/-30），`mvn -pl :moyue-account,:moyue-message -am test` → message 22 例 + account 18 例全绿，BUILD SUCCESS。
...
- 单测（纯 Mockito，贴合禁用 Docker 约束）：
  ...
  - **结果：`mvn -pl :moyue-account,:moyue-message -am test` BUILD SUCCESS**。

### 8.6 P1-1 AI 客服真实化 · 落地记录（2026-09-16）

> P1-1 已于 2026-09-16 落地：引擎接口升级为 `ReplyResult reply(ReplyContext)`，新增 `LlmReplyEngine`(@Primary) 配置驱动真实调用 OpenAI 兼容网关 + RAG 知识召回（经 `QaSearchClient` Feign 召回 `moyue_qa` topK）+ 多轮上下文 + 低置信转人工；LLM 未启用/缺密钥/异常时级联 `KeywordRuleReplyEngine` 兜底。提交 `f1ba0d6`（17 文件 +526/-15），`mvn -pl :moyue-ai,:moyue-search -am test` → search 70 例 + ai 10 例全绿，BUILD SUCCESS。
...
- **moyue-api-search**
  ...
  - 【新增】`client/QaSearchClient.java`(`@FeignClient(name="moyue-search", fallbackFactory=QaSearchClientFallbackFactory.class)`)：`GET /api/v1/internal/search/qa/retrieve` → `R<QaContextDTO>`。
...
- **RAG 走 Feign 而非既有 `SearchIndexClient`**：计划 §2 写「RAG 走 Feign 需新增 `QaSearchClient` 方法」；实际新增 `QaSearchClient` + 内部端点 + 降级工厂，`retrieveContext` 复用 `search()` topK；ES 不可用时降级工厂返回空知识库，不阻断客服（贴合禁用 Docker + ES 非硬依赖）。
...
- 单测（纯 Mockito）：
  ...
  - **结果：`mvn -pl :moyue-ai,:moyue-search -am test` BUILD SUCCESS**。
