...
public class UserService {
    ...
    /**
     * 实体转 DTO：对外接口（含 Feign UserClient）统一返回 DTO，与实体解耦。
     * 说明：UserEntity 当前已不含 password 字段，此处仍显式逐字段拷贝，
     * 保证后续实体新增敏感列（如 password、salt）不会随 JSON 泄露给调用方。
     * createTime / updateTime / isDeleted 属内部字段，DTO 未声明，故不映射。
     */
    ...
    /** 分页结果整体转 DTO */
    public static PageResult<UserDTO> toDtoPage(PageResult<UserEntity> page) {
        if (page == null) {
            return null;
        }
        ...
        result.setRecords(page.getRecords() == null ? null
                : page.getRecords().stream().map(UserService::toDto).toList());
        return result;
    }
}
